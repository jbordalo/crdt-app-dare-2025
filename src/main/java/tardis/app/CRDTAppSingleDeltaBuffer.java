...
public class CRDTAppSingleDeltaBuffer extends GenericProtocol {
	...
	private Logger logger = LogManager.getLogger(CRDTAppSingleDeltaBuffer.class);
	...
	private DeltaLWWSet deltaBuffer;
	...
	public CRDTAppSingleDeltaBuffer(Host myself) throws HandlerRegistrationException {
		super(CRDTAppSingleDeltaBuffer.PROTO_NAME, CRDTAppSingleDeltaBuffer.PROTO_ID);

		this.myself = myself;
		Peer peer = new Peer(myself.getAddress(), myself.getPort());
		this.crdt = new DeltaLWWSet(new ReplicaID(peer));
		initializeDeltaBuffer();
		...
		this.averageStateSize = registerMetric(
				new StatsGauge.Builder(CRDTAppSingleDeltaBuffer.STATE_SIZE_METRIC, Unit.BYTES).statTypes(StatType.AVG)
						.build());
		this.averageTimeMerging = registerMetric(
				new StatsGauge.Builder(CRDTAppSingleDeltaBuffer.TIME_MERGING_METRIC, "ms")
	...
	public void init(Properties props) throws HandlerRegistrationException, IOException {
		if (props.containsKey(PAR_BCAST_PROTOCOL_ID)) {
			...
			logger.debug(
					"CRDTAppSingleDeltaBuffer is configured to use broadcast protocol with id: " + this.bcastProtoID);
		} else {
		...
		if (this.generateWorkload) {
			...
			logger.debug("CRDTAppSingleDeltaBuffer has workload generation enabled.");
		} else
			logger.debug("CRDTAppSingleDeltaBuffer has workload generation disabled.");
	...
	private void handleAppWorkloadGenerateTimer(AppWorkloadGenerateTimer t, long time) {
		...
		if (new Random().nextDouble() > BUY_PROBABILITY) {
			...
			DeltaLWWSet delta = this.crdt.remove(sold);
			this.deltaBuffer.mergeDelta(delta);
	...
	private void handleBroadcastStateTimer(BroadcastStateTimer t, long time) {
		...
		if (this.testing) {
			...
			if (--this.roundsLeft == 0) this.executing.set(false);
		}
		...
		logger.debug("Sending delta of size {} bytes", stateSize);
		...
		// Clear delta buffer after sending
		initializeDeltaBuffer();
	}

	private void initializeDeltaBuffer() {
		Peer peer = new Peer(myself.getAddress(), myself.getPort());
		this.deltaBuffer = new DeltaLWWSet(new ReplicaID(peer));
	}
